...
import type { FixedObject, LevelData, StarSpot, ToolKind, Vec2 } from "./types";
...
function eggCountFor(level: number): number {
  if (level <= 1) return 1;
...
function toolBudget(level: number): Partial<Record<ToolKind, number>> {
  ...
  if (level >= 2) t.spring = 1 + +(level % 5 === 0);
  if (level >= 4) t.pad = 1 + +(level % 4 === 1);
  if (level >= 8) t.fan = 1 + +(level % 7 === 0);
  if (level >= 13) t.conveyor = 1 + +(level % 6 === 0);
  if (level >= 18) t.sticky = 1 + +(level % 9 === 0);
...
function placeHazardNearPath(
  ...
  for (const t of [along, along - 0.1, along + 0.1]) {
    for (const s of [220, 300, 160]) {
      ...
      const ok = ![start, basket, ...stars].some((q) => Math.hypot(q.x - p.x, q.y - p.y) < 90)
...
export function generateLevel(n: number): LevelData {
  ...
  const inkLimit = clamp(Math.round(dist * 1.55 + 260 + eggCountFor(level) * 40), 1100, 2100);
  ...
  if (level >= 30) {
    fixed.push(makeObj("sticky", 500, 930, { w: 160, h: 66 }));
  }
  if (level >= 38) {
    fixed.push(
  ...
  if (fireAt && level >= 6) {
    fixed.push(makeObj("fire", fireAt.x, fireAt.y, { w: 118, h: 86 }));
  }
  if (level >= 3) {
  ...
  return {
    ...
    parTools,
    timeLimit: level < 12 ? 70 : level < 32 ? 62 : 55,
  };
}
